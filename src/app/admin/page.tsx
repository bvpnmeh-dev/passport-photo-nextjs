...
export default function AdminPage() {
  ...
  const processPhoto = async () => {
    ...
    try {
      ...
      // compressImageFile returns base64 string
      const finalBase64 =
        typeof compressedFile === "string"
          ? compressedFile
          : await fileToBase64(compressedFile as File);

      // Create watermark photo first to get photoUuid
      const signedUrlResponse = await fetch("/api/photo/get-signed-url", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ specCode: selectedSpec.specCode }),
      });
      ...
      // Upload photo and get photoUuid
      ...
      const photoUuid = uploadData.photoUuid;

      if (!photoUuid) {
        throw new Error("No photoUuid received");
      }

      // Use admin bypass to get processed photo without payment
      const bypassResponse = await fetch("/api/admin/bypass-payment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          photoUuid: photoUuid,
      ...
      if (!bypassResponse.ok) {
        const errorData = await bypassResponse.json();
        throw new Error(errorData.error || "Admin bypass failed");
      }

      const bypassData = await bypassResponse.json();

      setProcessedPhoto({
        photoUuid: bypassData.photoUuid || photoUuid,
        idPhotoUrl:
          bypassData.idPhotoUrl || bypassData.idPhotoTempResultPhotoUrl,
        issues: [],
      });
              ...
              <div className="mb-6">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const spec =
                      allPhotoSpecs[
                        e.target.value as keyof typeof allPhotoSpecs
                      ];
